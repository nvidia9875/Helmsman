...
- [-] **F-4** ソロ評価実験: 録音済み会議音声 × 2 セット (Helmsman ON / OFF) で動作差を記録
  ...
  - [x] **第 2 サイクル完了 (2026-05-17)**: v3 (cheap = Decision+Dissent mini、transcript replay) で v2 と同じ素材を比較
    - v3: 173 utt / 26 ticks / **34→3** candidates (8.8%) / Decision×2 + Dissent×1 / topics decided 3 / **$0.03** (1/6 of v2)
    - 質的発見: mini はノイズ候補を増やすが Arbiter が 31/34 を弾く。Steering×5 が全部弾かれる。Decision 5/5 → 2/5 (3H 戦略・青黄テンプレ・平日効果が漏れる)
    - **結論**: gpt-5.4 は Decision/Dissent に必要。`--cheap` は dev イテレーション用 (transcript replay と組み合わせ wall 70 sec)
  - [x] 比較レポート `docs/eval-results.md` 作成 (Zenn 記事 §10 / デモ動画 / README で引用可)
- [-] **F-5** メトリクス記録 (GAR / 介入受容率 / 決定構造化精度) ← ハーネスが metrics.json で自動集計済
  - [x] acceptance_rate: v1 53.8% / v2 33.3% を計測
  - [x] delivered_by_level + by_agent: 自動集計
  - [ ] 決定構造化精度: v2 の Decision 5 件を手動で「会議内の実決定」と照合 (5/5 一致確認済、ただし規範データ未整備)
