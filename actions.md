...
- [x] **TB-O.C6** ✅ Entra Portal で Bot app ロゴアップロード完了 (2026-05-30、240×240 船舵デザイン) — Teams 会議内の `<>` placeholder が船舵になる
  - 生成スクリプト: `/tmp/gen_entra_icon.py` (favicon.svg ベース、PIL でラスタライズ)
  - アップロード先: https://entra.microsoft.com/#view/Microsoft_AAD_RegisteredApps/ApplicationMenuBlade/~/Branding/appId/ef2737f1-37bf-4392-a108-70f53f585b6d
...
## ✨ Day 8.5.P — 世界水準ブラッシュアップ (調査ベース、2026-05-21 着手)

> 世界のトップ Meeting AI (Granola / Cluely / Read.ai / Otter / Fathom / Sembly / Spinach / Recall.ai / Hume EVI) + UI 語彙 (Linear / Vercel / Bloomberg Terminal) + アカデミック (CHI 2025 OAI) + Microsoft 公式語彙 (Frontier Firm Orchestrator pattern) を横断調査し `docs/brushup-proposals.md` に整理。本セクションは **「実際に手を入れるもの」だけ** を切り出し、5/22-5/27 の 6 日間に分散して組み込む。
>
> **採用基準** (3 条件すべて満たすもののみ採用):
> 1. 残 10 日で実装可能 (工数 ≤ 3h、ストレッチは ≤ 8h)
> 2. リスク低 (既存テストを壊さない / 動作中の機能を退行させない)
> 3. 既存 narrative (¥0.03/会議, 10 件決定, 5/5 topics, 8 agents + 6 段 Arbiter, AI が代弁する構造) を **強化** する。競合・希釈しない
>
> **判断ログ — 採用しなかったもの** (記録のため明示):
> - ~~Cluely 風 Coach パネル~~ → 既存 InterventionFeed と機能重複、UI 過密リスク
> - ~~Fathom 風 1-click ハイライト + 切り出し共有~~ → 1.5 日コスト過大、説得力に直結しない
> - ~~Cmd+K command palette~~ → ギミック寄り、デモ動画で価値が出にくい
> - ~~"Helmsman Score" (Read.ai 由来 0-100)~~ → 既存 ¥0.03 / 10 件決定の強いヘッドラインを薄める懸念。decided/topic 比 (= 1.0 = 5/5) のほうがすでに強い
> - ~~Recall.ai 移行~~ → 現行 Graph Calling で十分、移行リスク不要
> - ~~Hume EVI 実装~~ → 記事ロードマップ言及のみ (P-A7)、実装は提出後 Phase 6

### Phase P-A — 記事 & ポジショニング (5/22-5/27 の章執筆スロットに分散)

> Zenn 記事 `docs/zenn-article-draft.md` の **位置付け・説得力** を世界 SOTA に揃える。
> 平日朝の細切れ時間で消化可能。

- [x] **P-A1** §4.6 を **OAI フレーム (CHI 2025 Houtti et al.)** との明示的位置付けに書き換え ✅ 2026-05-21 完了 (`docs/zenn-article-draft.md` §4.6 全置換、OAI 3 段マッピング表 + future work 接続 + 「論文化は今後の課題」と honest 化)
  - 引用: [arxiv.org/abs/2501.10553](https://arxiv.org/abs/2501.10553) "Observe, Ask, Intervene: Designing AI Agents for More Inclusive Meetings" (68 名 12 グループ実験で OAI > 完全自律)
  - 主張構造: OAI の Ask/Intervene を Helmsman L1/L2/L3 に **構造化マッピング** + Density-aware silence + Authority Gradient + Mode-conditional の 3 軸で「実運用化」したのが本作の新規性
  - 工数: 30 分
- [x] **P-A2** §3.1 (Agentic AI とは) に **Microsoft Work Trend Index 2025 "Frontier Firm"** 4 work patterns (Author / Editor / Director / Orchestrator) 対応表を追加 ✅ 2026-05-21 完了 (§3.1.1 を 1 表 + 短文で挿入、「本論は独自設計で進む」明示で希釈リスク回避)
  - 引用: [microsoft.com/en-us/worklab/work-trend-index/2025-the-year-the-frontier-firm-is-born](https://www.microsoft.com/en-us/worklab/work-trend-index/2025-the-year-the-frontier-firm-is-born)
  - 主張: Helmsman = **Orchestrator pattern の個人開発リファレンス実装** (8 agent 並列 + 人間が L1-L3 を承認/却下)
  - 工数: 30 分
- [x] **P-A3** §8 (Facilitator 比較) を **5 列** (Granola / Cluely / Read.ai / MS Facilitator / Helmsman) に拡張 ✅ 2026-05-21 完了 (公開価格 + 公開機能ベースで中立トーン、宣伝図化リスク回避、§8.1 各社との関係 / §8.2 Helmsman を選ぶケース に分割)
  - 軸: 介入の能動性 / メタロジック / multi-agent 数 / 価格 / OSS
  - Helmsman の独自性 (音声 + 6 段フィルタ + 8 並列 + MIT) を視覚的に確立
  - 工数: 1 時間 (調査済データを `docs/brushup-proposals.md` D-4 に整理済)
- [x] **P-A4** Zenn 冒頭 (TL;DR の前 or §1 直後) に **「2026 AI Meeting カテゴリマップ」** 1 枚図解 ✅ 2026-05-21 完了 (§1.1.1 を mermaid quadrantChart で実装、9 製品 [Granola/Otter/Fathom/Fireflies/tl;dv/Sembly/Read.ai/MS Facilitator/Cluely/Helmsman] プロット、「公開機能ベースで筆者整理」と注記し宣伝図化リスク回避)
  - 4 象限: 横軸 = 事前 / 最中 / 事後、縦軸 = 受動 / 能動
  - 既存プロダクトをプロットし Helmsman = 唯一の「最中 × 能動」象限を視覚化
  - Excalidraw → PNG 埋め込み (or mermaid quadrantChart)
  - 工数: 30 分
- [x] **P-A5** §6 の実装ハイライト 9 連から **「4 つの盛大に間違えた瞬間」(失敗集約)** を独立小節として再編 ✅ 2026-05-21 完了 (§6.0 サマリ追加、4 件 [rate_limit / cheap 逆転 / TeamsMeetingLinkLocator / memo 過信] を表化、既存 §6.1-6.9 はそのまま backlink 構造)
  - 対象: ACS TeamsMeetingLinkLocator 不在事件 / Arbiter rate_limit wall-clock バグ / cheap vs default 逆転 / Decision Capture memo 過信
  - スタイル: Anthropic "Building Effective Agents" の "What didn't work" を踏襲
  - 個人開発者が読みたくなる構造、Zenn のいいねブースター
  - 工数: 2 時間
- [x] **P-A6** Microsoft 公式語彙の **リンク監査 + 脚注集約** ✅ 2026-05-21 完了 (Appendix B を 4 セクション化: リポジトリ内資料 / Microsoft 公式 / 学術 [CHI 2025 OAI + Anthropic] / 他社 Meeting AI)
  - Frontier Firm / Multi-Agent Orchestration (Build 2025) / Intelligent Recap / Facilitator / Copilot Studio
  - 審査員は公式語彙でレビューするので、明示リンクで「ちゃんと読んでる」を証明
  - 工数: 1 時間
- [x] **P-A7** §12 (ロードマップ) Phase 6 Multimodal に **Hume EVI** 統合の 1 段落追記 ✅ 2026-05-21 完了 (表に行追加せず Phase 6 補足扱いで挿入、「未実装・API 検証/コスト/倫理レビュー未済」明示、§4 Arbiter + §5 代弁構造との 3 軸統合ストーリーで本論と接続)
  - 緊張時は柔らかいトーン / 切迫時はキビキビと、prosody を感情に合わせて変える方向
  - 引用: [hume.ai/empathic-voice-interface](https://www.hume.ai/empathic-voice-interface)
  - 工数: 15 分

### Phase P-B — UI ブラッシュアップ (5/24-5/26、デモ動画撮影 5/28 前に完了)

> デモ動画と Zenn 冒頭スクショで「世界の語彙を踏まえた UI」を 5 秒で伝える。
> 既存 component を **拡張** するのみ、新ルート/新エンドポイント追加なし。

- [x] **P-B1** **`apps/web/src/components/RightNowStrip.tsx`** 新規作成 ✅ 2026-05-21 完了 (新規 component 200 行 + MeetingRoom 配線 + vitest 3 件 [STAND BY / LIVE+speaker / delivered count] + 型 OK + Griffel warning 解消)
  - 位置: `MeetingRoom.tsx` の `<KpiRow>` 直上、`<BotStatusStrip>` の下
  - 表示: 1 行 / 高さ 32px / monospace
  - 内容例: `● LIVE  Listening · {speaker} speaking · last decision {n}min ago · next nudge in {sec}s`
  - データソース: `meeting.bot_status` + `transcript.utterances[last]` + `delivered_interventions[last].delivered_at` (既存 polling データで完結、新 API なし)
  - スタイル: 既存 `.glow-active` + monospace で Granola/Linear "right-now bar" パターン
  - 受け入れ条件: bot offline 時は dim 表示、in_call 時は pulsing
  - 工数: 2-3 時間
- [x] **P-B2** **`ReportPanel.tsx` プレビュー部の 2 色レンダリング** (Granola "augmented notes" 由来) ✅ 2026-05-21 完了 (`apps/web/src/lib/augmented.ts` 純粋関数として切り出し + ReportPanel preview を行単位 span に再構成 + memo 非空時のみ凡例表示 + vitest 8 件 + 既存テスト全 28 件 pass)
  - memo の各行 (substring match) → text-1 (白)
  - それ以外の Helmsman 由来 → text-2 (灰)
  - quote (`> ...`) → 既存 border-left accent
  - 実装: 生成済み markdown を行単位で split → memo source の line tag 付け → CSS class 切替
  - 主張: 「あなたが書いたことを Helmsman は塗り直さない」の "Invisible AI" 哲学
  - 受け入れ条件: memo 空の時は単色 (既存挙動), memo 与えた時のみ 2 色化
  - 工数: 2-3 時間
- [x] **P-B3** **`MeetingPulse.tsx`** の「参加者の動き」card に **Gini 係数 (発言量の偏り)** 追加 ✅ 2026-05-21 完了 (`apps/web/src/lib/gini.ts` 純粋関数 + 3 段ラベル [均等/軽い偏り/偏重] + MeetingPulse 参加者カードヘッダに equity row 配線 + vitest 8 件 [empty/equal/zero/skewed/sorted/clamped/bands/labels])
  - 計算: `computeGini(participants.map(p => p.utteranceCount))` で 0.0-1.0
  - 表示: 1 数字 + ラベル (`< 0.2` 平等 / `0.2-0.4` 軽偏り / `> 0.4` 偏重)
  - Read.ai の "participation equity" / Helmsman の Quiet Activator z-score と整合
  - 既存 z-score Quiet Activator との整合性を保つ説明文 (`MeetingPulse` 既存 sub テキスト) を 1 行追加
  - 工数: 1.5 時間

### Phase P-C — スクショ & デモ動画組み込み (5/28-5/29)

- [x] **P-C1** デモ動画の冒頭 5 秒に **P-B1 (Right Now strip) のクローズアップ** を追加 ✅ 2026-05-21 完了 (`docs/demo-numbers.md` に 3 cut 追記: Cold open 直後の "Hook clip 5 秒" + Demo 60 秒中の "Equity cut 4 秒" + Report scene 内の "Augmented notes クローズアップ")
  - `docs/demo-numbers.md` の撮影台本に「右上 strip の pulsing dot + LIVE バッジ」を 1 cut 追加
  - 「いま動いている感」が 5 秒で伝わる
- [ ] **P-C2** Zenn 記事冒頭 (TL;DR の直前) に **新 UI スクショ 1 枚** を埋め込み
  - 構図: Right Now strip + KPI 行 + InterventionFeed + LiveTranscript が 1 画面に収まる、1280×720
  - スタイル: Anthropic / Granola 流の "what it looks like" 冒頭
  - 工数: 30 分 (UI が確定してから撮影)

### Phase P-D — ストレッチ (時間が余れば、判断は 5/27 朝)

> どれも価値はあるが、デモ動画と Zenn 推敲を優先するため**条件付き**。
> 5/27 朝に「Zenn と P-B 系がすでに 90% 完了」が確認できた場合のみ着手、それ以外は提出後 Phase F へ逃す。

- [ ] **P-D1** (Stretch) 簡易 **Agent Trace ビューア** — `/eval-trace?run=<id>`
  - `eval_runs/<id>/` 配下の JSONL を読んでブラウザ表示
  - レイアウト: 横軸 = 時間軸 (60 分会議)、縦軸 = 8 agent、各セルに candidates + Arbiter 判定理由
  - フル機能ではなく **静的 viewer** で十分 (LangSmith/Sentry AI Agents 最小実装)
  - 価値: Zenn §6.7「30 秒で再現できる」を実画面で証明できる
  - 工数: 6-8 時間
  - 5/27 朝に GO/NO-GO 判断 → NO なら Phase F に明示移送
- [ ] **P-D2** (Stretch) **Sprint Mode テンプレ** (Spinach.io 流)
  - CreateMeeting のゴール欄に 5 種プリセット: Sprint Planning / Daily / Retro / 1on1 / 顧客 Discovery
  - 各テンプレで goal + topics + 時間配分が prefill
  - 工数: 3 時間
- [ ] **P-D3** (Stretch) **/meetings/{id}/glance** Glance View (Sembly 流)
  - A4 1 枚に要約 (ゴール / topics / 決定 10 件 / 未解決 concerns)
  - `window.print()` で PDF 化、社内 Slack 共有が捗る
  - 工数: 3 時間

### Phase P 完了基準

- [ ] P-A1〜P-A7 全完了 → Zenn 記事が世界 SOTA との位置付けで読める状態
- [ ] P-B1〜P-B3 全完了 → UI が Granola/Linear/Read.ai 由来の語彙を備える
- [ ] P-C1〜P-C2 完了 → デモ動画と記事冒頭で新 UI が映る
- [ ] P-D* → 着手判断を 5/27 朝に記録 (どれを採用したか、Phase F に逃したか明示)
- [ ] 採用ログ更新 — どの提案が記事/実装に反映されたか `docs/brushup-proposals.md` の F. ロードマップ表に進捗欄追加
...
- [x] **D-1** 撮影台本の最終確定 ✅ 2026-05-30: `docs/demo-storyboard.md` v2 完成 (9 シーン / 2 人会議 / 物理レイアウト図 / ナレーション原稿 / フォールバック / 当日チェックリスト)
- [x] **D-2** ソロ撮影セットアップ ✅ 2026-05-30: 構成変更 → **PC 2 台 (Teams 参加) + スマホ 2 台 (yamada.wav / sato.wav 同時再生) + 録画 PC 1 台**。被り 0 検証済の 2 人会議 fixture (`demo_meeting_2person.jsonl`, 32 発言, 3:53) + 音声 (Daichi 男声 / Mayu 女声) を準備
- [ ] **D-3** デモ動画撮影 (5/31 予定) ← 上記の物理セットアップで通し撮影
- [ ] **D-4** 動画編集 + 字幕 (5/31 PM、iMovie で 3:53 → 3:00 にカット)
- [x] **D-S1** 擬似会議 fixture + TTS 音声生成スクリプト ✅ `scripts/gen_demo_audio.py` (2 人 + ミックス対応、被り検査付き)
- [x] **D-S2** Helmsman ダッシュボード反応テスト ✅ `eval_runs/20260530-120221-audio-test/` で combined.wav → STT → 5 件介入 + 5 件決定捕捉確認 ($0.0118)
- [x] **D-S3** eval runner の L3 発火修正 ✅ `audio-time` と `wall-clock` を同期する 1 行追加 (`src/helmsman/eval/runner.py`) で text eval で 1 L3 確認
...
Day 8.5.P (世界水準ブラッシュアップ):    11  / 14   進行中 (P-A 7/7 + P-B 3/3 + P-C 1/2 完了。残: P-C2 [スクショ撮影 = UI 動作後 user 操作要] + P-D 3 stretch [5/27 朝判断])
...
**残タイムライン (5/22〜6/1、8.5.P 統合済)**:

| Day | Date | 内容 |
|---|---|---|
| - | 5/22 (木) | 軽日 + USER タスク消化 (Entra logo / ACS リソース削除) + **P-A1** (OAI フレーム §4.6, 30分) + **P-A2** (Frontier Firm §3.1, 30分) + **P-A4** (象限マップ作図, 30分) |
| - | 5/23 (金) | 🔴 USER: 8.5.N.C 実行 (`apps/teams-app/SIDELOAD.md` 通り) + **P-A6** (Microsoft リンク監査, 1h) + **P-A7** (Hume EVI ロードマップ, 15分) |
| - | 5/24 (土) | Zenn 章立て (Graph Calling 編) + **P-B1** (RightNowStrip 実装, 2-3h) |
| - | 5/25 (日) | Zenn 続き (8 agents + Arbiter 編) + **P-A3** (5 列比較表, 1h) + **P-B2** (Report 2 色化, 2-3h) |
| - | 5/26 (月) | Zenn 続き (TTS pause/resume + recording loop 編) + **P-B3** (Gini 係数, 1.5h) |
| - | 5/27 (火) | Zenn 推敲 (cheap mode 優位性編) + **P-A5** (失敗集約章, 2h) + **P-D 着手判断** (Trace viewer or Sprint テンプレ or Glance View、状況次第) |
| - | 5/28 (水) | **デモ動画撮影 Day 1** — `docs/demo-numbers.md` の台本通り + **P-C1** (Right Now strip クローズアップ追加) |
| - | 5/29 (木) | デモ動画撮影 Day 2 + 編集 + 数値スライド合成 + **P-C2** (Zenn 冒頭スクショ埋め込み, 30分) |
| - | 5/30 (金) | 動画完成 + Zenn 公開 + 最終 QA (Phase P 完了基準の確認) |
| - | 5/31 (土) | 提出フォーム入力 + final smoke test |
| - | 6/1 (月) | 🚀 提出 |

**Phase P タスクのデイリー工数試算**:
- 5/22: A1+A2+A4 = 1.5h
- 5/23: A6+A7 = 1.25h (USER タスクと並行可)
- 5/24: B1 = 2-3h
- 5/25: A3+B2 = 3-4h
- 5/26: B3 = 1.5h
- 5/27: A5 (2h) + P-D 判断
- 5/28: C1 = 撮影時組み込み (新規工数小)
- 5/29: C2 = 0.5h
- **合計**: 必須分 11-14h を 8 日に分散、Zenn 執筆と並行可能
...
**P0 (必須、shun さん手動が必要)**:
- [x] **TB-O.C6** ✅ Entra Portal で bot app ロゴアップロード完了 (2026-05-30)
