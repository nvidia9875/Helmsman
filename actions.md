...
- [ ] **Az-8** Azure Communication Services 作成 ← L3 Speak フェーズで実装
- [ ] **Az-9** Azure AI Search (Basic) 作成 ← RAG 不要なフェーズではスキップ
- [x] **Az-10** Azure Key Vault 作成 + シークレット集約
- [x] **Az-11** Application Insights + Log Analytics 作成
- [ ] **Az-12** Microsoft Entra ID アプリ登録（SPA タイプ）← Frontend ログイン実装時
  - [ ] `client_id` 取得
  - [ ] Graph API スコープ: User.Read
- [ ] **Az-13** GitHub Actions に Azure 認証（Service Principal）追加 ← CI/CD デプロイ時
...
- [ ] **BE-8** 各エージェントのユニットテスト ← 次フェーズで pytest セットアップ
- [ ] **BE-9** Azure Speech リアルタイム STT 統合 ← Phase 3 で実装（Web Speech API でフォールバック中）
...
- [ ] **FE-4** Microsoft Entra ID Web 認証（MSAL.js）統合 ← 任意（dev は localStorage で代用）
...
- [ ] **MVP-4** Microsoft Planner 連携 or 手元 export 機能
...
- [ ] **TB-C5** UI に「🔊 音声で介入する」L2 → L3 昇格ボタン ← smoke test 後に追加
...
- [ ] **F-1** Power BI ダッシュボード（オプション）
- [ ] **F-2** Application Insights のログ整備
- [ ] **F-3** エラーハンドリング全面強化
...
## 進捗集計 (2026-05-17 22:00 更新)

```
Day 0  (5/16 セットアップ):              7  /  7   完了
Day 1  (Azure リソース):                12  / 14   完了 (CommSvc 追加で 12/14 に。Functions/Entra/SP は不要化)
Day 2-5 (Backend Core):                 15  / 17   完了
Day 6-7 (Frontend):                      9  / 10   完了
Day 8   (Web MVP)                        4  /  4   ✅ 前倒し達成
Day 8.5.A (テスト整備):                  3  / 11   完了 (T-1/2/3 + 拡張で 5/11 が実質)
Day 8.5.B (会議継続性 C-1〜6):           6  /  6   ✅ 完了
Day 8.5.C (文書 RAG DOC-1〜4 + 7):       5  /  9   完了 (DOC-5/6/8/9 次フェーズ)
Day 8.5.D (審査基準):                    1  /  8   完了 (F-3 のみ完了。B-1/2/3, A-1/2, F-1/2/4/5 残)
Day 8.5.E (コストダッシュボード):        6  /  7   完了 (COST-7 のみ残)
Day 8.5.F (Teams Bot Phase A+B):        12  / 26   完了 (Phase A 6/6 + B 6/6、C 5、D 4、E 5 残)
Day 9-11 (PWA/声紋/Hybrid):              ❌ 不要化 (Teams Bot ピボットにより)
Day 12 (L3): → 8.5.F Phase C            -- L3-1〜3 は TB-C に統合、L3-4/5 は Phase E
Day 13-15 (仕上げ/デモ/Zenn):            0  / 17   未着手
Day 16  (提出):                          0  /  5   未着手
─────────────────────────────────────
Total: ~80 / ~141 完了 (~57%)
```

**Teams Bot ピボット後の新しい northern star**:
- 5/19 trial 開始 → smoke test → Phase C (TTS) → Phase D (UI) → デモ撮影 (5/30)
- 5/17 時点で本番デプロイ完了 (Container App + SWA) + Phase A/B コード完成、Phase E smoke test 待ち
