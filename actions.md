...
- [-] **F-4** ソロ評価実験: 録音済み会議音声 × 2 セット（Helmsman ON / OFF）で動作差を記録 ← **評価ハーネス `scripts/eval_offline.py` 実装済**、音声/transcript 投入待ち
- [-] **F-5** メトリクス記録（GAR / 介入受容率 / 決定構造化精度）← ハーネスが metrics.json で自動集計 (acceptance_rate / delivered_by_level/agent / topic_states / cost)、実音声で計測待ち
