...
#### 1.1.1 2026 AI Meeting カテゴリマップ

世界の主要 9 製品を **「タイミング軸 (事後 ← → リアルタイム)」 × 「能動性軸 (受動観測 ← → 能動介入)」** の 2 軸でプロットすると、構造的な空白が見えます。
...
- **左下 (事後 × 受動)** = Granola / Otter / Fathom / Fireflies / tl;dv / Sembly。要約・検索・アクション抽出は完成
- **右下 (リアルタイム × 受動)** = ほぼ空白。可視化に留まる
- **左上 (事後 × 能動)** = ほぼ空白
- **右上 (リアルタイム × 能動)** = Cluely (個人向け sales coach) / MS Facilitator (テキスト中心) / Read.ai (engagement signal) が薄く存在。**Helmsman はこの象限で「8 agent 並列観測 + L1/L2/L3 グラデーション介入 + 音声発話」まで踏み込んだ唯一の作品**

> 各製品の座標は公開機能ベースで筆者が整理したもので、厳密な定量比較ではありません。詳細な機能対比は §8 で 5 列表として展開します。
...
#### 3.1.1 Microsoft "Frontier Firm" 4 patterns との位置付け

Microsoft Work Trend Index 2025 [Frontier Firm](https://www.microsoft.com/en-us/worklab/work-trend-index/2025-the-year-the-frontier-firm-is-born) は、人間と AI の協業を 4 つの work pattern で整理しています。本作は **Orchestrator pattern** に該当します。

| Pattern | 主導 | 例 |
|---|---|---|
| Author | 人間が書く / AI 補助 | 議事録手入力 (旧来) |
| Editor | AI 初稿 → 人間が編集 | Otter / Granola の事後要約 |
| Director | 人間が仕様 → AI が実行 | Helmsman の Report 生成 (§11) |
| **Orchestrator** | **複数 agent 並列、人間は例外時のみ承認/却下** | **Helmsman 本体 (会議中)** |

会議中、人間 (chair) は L1/L2/L3 介入を承認/却下するだけで、8 agents が並列で観測・分析を続けます。

> 本記事の以降の章では公式語彙ではなく独自設計 (Arbiter 6 段フィルタ / 代弁構造 / Authority Gradient 等) で論を進めます。Frontier Firm は **位置付け** のための参照枠であり、本作の新規性そのものではありません。
...
### 4.6 既存研究との位置付け ― CHI 2025 OAI フレームを実運用化する

最も近い先行研究は CHI 2025 の **"Observe, Ask, Intervene" (OAI)** フレームです (Houtti, Zhou, Terveen, Chancellor / University of Minnesota / [arxiv.org/abs/2501.10553](https://arxiv.org/abs/2501.10553))。9 回のアイデアセッション + 68 名 12 グループのプロトタイプ実験 + 18 名のインタビューを通じて、参加者は AI agent が**介入前に質問する** ことを好み、完全自律よりも OAI を選好した、というのが主結論です。

Helmsman の Arbiter + L1/L2/L3 グラデーションは、OAI フレームを **「具体的な実装パラメータ + 観測可能な品質メトリクス」** に翻訳した位置にあります。

| OAI フレーム (CHI 2025) | Helmsman の対応実装 |
|---|---|
| **Observe** (発言/参加状況を観察) | 8 agents が並列で観測 (Coverage / Steering / Quiet / Dissent / Decision / TimeKeeper …) |
| **Ask** (介入候補をユーザーに見せ確認) | **L1 ささやき** (chair-only) + **L2 サイドバーカード** (全員に提案、人間が承認) |
| **Intervene** (実際に発火) | **L3 音声介入** (Bot が会議で日本語発話) |

OAI 論文では「いつ Ask / Intervene すべきか」のタイミング制御は future work として残されています。本作の **Density-aware silence + Authority Gradient + Mode-conditional + User Intensity** の 4 軸 (§4.3 / §4.4) は、この future work を**実装で具体化** したものに該当します。

論文化の射程: CHI 2025 OAI は **「介入の有無」と「介入前の確認の有無」** を主軸に置きました。本作で次に問えるのは **「介入の強度をどう設計するか (L1/L2/L3 グラデーション)」** と **「Authority Gradient のような社会的非対称性をどうフィルタに織り込むか」** であり、会議運営研究と推薦システムの境界領域として、CHI / CSCW で扱える問題設定だと考えています (本記事は実装報告であり、論文化は今後の課題)。
...
### 6.0 サマリ ― 4 つの「盛大に間違えた瞬間」

実装で本当に痛かったのは、論文化を狙えそうな §4 Arbiter の設計ではなく、もっと泥臭い 4 つの瞬間でした。先に種明かしすると、Helmsman が今動いているのはこの 4 つを潰したからです。

| # | 何を間違えたか | どこで気付いたか | 効いた修正 |
|---|---|---|---|
| 1 | **Arbiter rate_limit が wall-clock 依存** — offline replay で 25 分会議を 70 秒で再生したら 2 件目以降が全部 drop されていた | eval ハーネス v3 が「mini は品質劣化」と誤った結論を出した瞬間 | `decide(now=...)` で時刻 inject、eval は audio time を渡す → 介入数 3 → 15 (§6.1) |
| 2 | **「高い tier = 高品質」と信じていた** — gpt-5.4 (HIGH) で動かしていたら cheap (mini) のほうが decisions 2×、コスト 1/6 で勝ってしまった | 第 3 サイクル eval で逆転発覚 | 全 agent 毎に tier を選べる設計に統一、本番推奨を `--cheap` に格上げ (§6.2) |
| 3 | **ACS の `TeamsMeetingLinkLocator` が SDK 全バージョンに存在しない** — Microsoft 公式 docs を信じて実装、本番 ImportError | Day 5/19 に LLM が出してくれた API 名が偽だった | Service-hosted Graph Calling Bot に方針転換、Application Access Policy を PowerShell で設定 (§6.4) |
| 4 | **Decision Capture が memo を過信** — memo に書かれていない決定事項を memo 由来として書く hallucination | Report agent の smoke 検証 Case 3 | 「ルール #5: 推測で事実を書かない、確実な情報源に基づくものだけ」追加 → memo の不確実な部分まで `要確認` で残すようになった (§3.8 末尾) |

> 4 件とも、**実装側のロジック設計でしか直せない問題** でした。プロンプトを丁寧にするとか LLM を強くするとかでは解けない。Agentic AI のデバッグは「LLM がおかしい」ではなく「人間側の制御設計がおかしい」ケースのほうがずっと多い、というのが本作の最も大きな学びです。
...
## 8. 世界の主要 Meeting AI との位置付け

§1.1.1 で 9 製品を 2 軸でプロットしましたが、ここでは **代表 4 製品 (Granola / Cluely / Read.ai / MS Facilitator)** との 5 列詳細比較を、各社の **公開機能 + 公開価格** ベースで整理します。

> 価格は 2026-05 時点で各社公開ページから引用。製品仕様は今後変更される可能性があります。各社プロダクトは Helmsman とは別カテゴリ (事後/個人 coach/分析) で完成度が高く、本表は **代替関係ではなく座標差を示すもの** です。

| 軸 | [Granola](https://www.granola.ai/pricing) | [Cluely](https://cluely.com/pricing) | [Read.ai](https://www.read.ai/meeting-tools) | [MS Facilitator](https://learn.microsoft.com/ja-jp/microsoftteams/facilitator-teams) | **Helmsman 🧭** |
|---|---|---|---|---|---|
| **主戦場** | 個人ノート (事後要約) | 個人 sales coach (リアルタイム) | 会議分析 (engagement / sentiment) | Teams 会議補佐 (テキスト) | チーム会議の能動介入 (音声+テキスト) |
| **介入タイミング** | 事後 | リアルタイム (個人へ) | リアルタイム + 事後 | リアルタイム + 事後 | リアルタイム |
| **介入対象** | 本人のみ | 本人のみ (画面オーバーレイ) | 会議主催者 (dashboard) | 会議全体 (Teams chat) | **L1=chair / L2=全員 / L3=音声で会議全体** |
| **AI 構成** | 単一 (要約) | 単一 (coaching) | 単一 (engagement model) | 単一 (Copilot) | 8 並列 + Arbiter |
| **介入のメタロジック** | — | 個人 playbook | engagement signal | — | Density-aware + Authority Gradient + Mode-conditional + User Intensity |
| **配布形態** | デスクトップ / ブラウザ | デスクトップオーバーレイ | bot / 拡張機能 | M365 Copilot 内蔵 | Teams bot + Web Tab |
| **音声で会議に発話** | ❌ | ❌ | ❌ | ❌ (テキストのみ) | ✅ Azure TTS Nanami |
| **公開価格 (個人/業務)** | $0 / $14-35 | $19 / $49 | $0 / $19.75 | M365 Copilot $30 + M365 SKU | Azure 従量 (実測 $0.03/会議) |
| **配布ライセンス** | クローズド | クローズド | クローズド | クローズド | **MIT OSS** |

### 8.1 各社との関係 (代替か、補完か)

- **Granola との関係**: 補完。Granola は事後要約に特化、Helmsman は会議中の介入に特化。**会議中 Helmsman + 会議後 Granola** という同居運用は十分ありえます (要は両ツールが録音権限を競合しないよう設計次第)
- **Cluely との関係**: 主戦場が違う。Cluely は **個人向け** の "セールス商談 / インタビューで自分が何を言うべきかを画面に出す" 設計、Helmsman は **会議全体の動的を制御** する設計。1on1 商談なら Cluely、複数人会議なら Helmsman
- **Read.ai との関係**: engagement signal の可視化と相性が良い。Read.ai が「いま全体の sentiment が下がっている」を出し、Helmsman が「だから Dissent Surface を発火」と動く、というセンサー+アクチュエータの分業も想像できます (未統合)
- **Microsoft Teams Facilitator との関係**: **両者は同じ会議で同時動作可能**。Facilitator がノート / Q&A / タイムラインマーカーを担い、Helmsman が議論ダイナミクスの能動介入と決定捕捉を担う、というハイブリッド運用が自然です。M365 Copilot 既導入の組織には Facilitator + Helmsman の二刀流、未導入には Helmsman 単体、という導入経路がありえます

### 8.2 Helmsman を選ぶ典型ケース

- **複数人会議で能動介入が欲しい**: Granola / Read.ai は事後 / engagement 可視化のみ
- **音声で会議に介入してほしい**: Cluely / Facilitator はテキスト中心
- **外部参加者を含む会議**: Facilitator は M365 Copilot ライセンス保持者のみが恩恵を受ける構造、Helmsman は Teams 会議への ACS External 参加で外部参加者がいても動く
- **OSS でカスタマイズしたい**: 上記 4 製品はいずれもクローズド
...
#### 補足: Phase 6 Multimodal で検討中の Hume EVI 統合 (未実装)

現在の TTS は Azure Speech `ja-JP-NanamiNeural` の固定 prosody です。実会議では「議論が緊張している瞬間に明るすぎる声で介入する」と空気が崩れる懸念があり、声色 (tone / rhythm / timbre) を会議の状態と整合させる発展が必要だと考えています。

[Hume EVI (Empathic Voice Interface)](https://www.hume.ai/empathic-voice-interface) は会話の感情状態に応じて TTS prosody を動的に変調する API です。Phase 6 のマルチモーダル拡張 (表情認識 Engagement Index) と組み合わせると、Helmsman の介入は次の 3 軸で会議と同期します:

- **何を言うか** ← §4 Arbiter (6 段フィルタ)
- **誰に向けて言うか** ← §5 代弁構造 + L1/L2/L3 グラデーション
- **どんな声色で言うか** ← Hume EVI (将来統合検討)

※ **本ハッカソン提出版 (v1.0.0) では未実装**。ロードマップ表本体ではなく補足扱いにしているのは、現時点で API 検証 / コスト試算 / 倫理レビュー (感情応答 TTS は別の Responsible AI 課題を生む) のいずれも済んでいないためです。
...
- 🌐 世界 SOTA とのブラッシュアップ提案: [`docs/brushup-proposals.md`](https://github.com/nvidia9875/Helmsman/blob/main/docs/brushup-proposals.md)

### Microsoft 公式リファレンス (本文で位置付け参照)

- [Microsoft Work Trend Index 2025: The year the Frontier Firm is born](https://www.microsoft.com/en-us/worklab/work-trend-index/2025-the-year-the-frontier-firm-is-born) — §3.1.1 Orchestrator pattern の参照元
- [Microsoft Build 2025: Copilot Studio Multi-Agent Orchestration](https://www.microsoft.com/en-us/microsoft-copilot/blog/copilot-studio/multi-agent-orchestration-maker-controls-and-more-microsoft-copilot-studio-announcements-at-microsoft-build-2025/) — §3.3 で採用しなかった理由を述べた対象
- [Microsoft Teams Facilitator (Learn)](https://learn.microsoft.com/ja-jp/microsoftteams/facilitator-teams) — §8 比較対象
- [Microsoft Teams Intelligent Recap (Learn)](https://learn.microsoft.com/en-us/microsoftteams/intelligent-recap-calls-meetings) — §1.1 で事後 AI として参照
- [Microsoft Responsible AI Standard v2](https://blogs.microsoft.com/wp-content/uploads/prod/sites/5/2022/06/Microsoft-Responsible-AI-Standard-v2-General-Requirements-3.pdf) — §9 で 6 原則対応をマッピング
- [Azure OpenAI Service / Azure AI Speech / Azure Communication Services / Azure Container Apps / Cosmos DB / AI Search](https://azure.microsoft.com/) — §2.2 物理コンテナ構成

### 学術 (本文で引用した先行研究)

- Houtti, M., Zhou, M., Terveen, L., Chancellor, S. (2025). **"Observe, Ask, Intervene: Designing AI Agents for More Inclusive Meetings"**, CHI '25. [arxiv.org/abs/2501.10553](https://arxiv.org/abs/2501.10553) — §4.6 で本作 Arbiter のフレーミングに使用
- Anthropic Engineering, **"Building Effective Agents"**: [anthropic.com/research/building-effective-agents](https://www.anthropic.com/research/building-effective-agents) — §3.1 で Agentic AI 定義に使用

### 他社 Meeting AI (§1.1.1 象限マップ + §8 比較表で参照、公開情報出典)

- [Granola — AI Notepad](https://www.granola.ai/) / [pricing](https://www.granola.ai/pricing)
- [Cluely — Live AI Meeting Assistant](https://cluely.com/) / [pricing](https://cluely.com/pricing)
- [Read.ai — Meeting Tools](https://www.read.ai/meeting-tools)
- [Otter.ai](https://otter.ai/) / [Fathom](https://www.fathom.ai/) / [Fireflies](https://fireflies.ai/) / [tl;dv](https://tldv.io/) / [Sembly AI](https://www.sembly.ai/)
- [Hume EVI (Empathic Voice Interface)](https://www.hume.ai/empathic-voice-interface) — §12 ロードマップで参照
