...
- **どう作ったか**: Azure の上で **10 個の AI** がそれぞれ違う観点から会議を見ていて、「司会の補佐役」がそれをまとめて、ちょうどいい強さで助け舟を出します。強さは「ささやき」「カード」「声で発言」の 3 段階です。**先週の会議で何を決めたか覚えていてくれる「記憶 AI」** と、**カメラも声色も使わず発言テキストだけで「いま誰がどんな気持ちで話してるか」を読み取る「感情 AI」** も組み込みました(顔シグナル路線は途中で撤退、§9.2 にその経緯を書いています)。
...
sequenceDiagram
    ...
    par 並列実行
        ...
        H->>G: MemoryRetriever (Phase 7)
        H->>G: ToneAgent (Phase 8、cache hit でスキップ)
...
- **Helmsman は、ここに「9 人並列で見守る + 3 段階の助け舟 + 音声で発言 + 発言テキストから感情を読む」まで踏み込んだ、わりと珍しい立ち位置です**
...
- **壁 1: Teams 会議の全員の映像は、Python の bot からは取れない**。Microsoft 公式の media SDK は C# 専用で、Windows host も実質必須です。既存の Python + Linux container 基盤と相性が悪く、ハッカソン期限内に書き直すリスクが高すぎる
...
副産物として、frontend bundle が **約 5MB 軽くなりました** (MediaPipe WASM + model file を削除)。PWA の初回ロードが速くなり、低速回線での体感も良くなっています。
...
- **LiveTranscript**: 各発言の右に絵文字バッジ ("👍 賛同" "😕 懸念" 等) と話者名。bot が会議参加者リストを持っていれば **`u-abc12345` のような UUID ではなく実名 (山田 / 田中) で表示** されます (`Participant.display_name` を ToneAgent が cache に持つ実装)
- **ParticipantsPanel**: 話者ごとに「主に積極的 😊」「困惑気味 😕」などの傾向バッジ、**直近 8 発言の感情をカラードットで流す stream**、最新の発言テキスト、sample 数 + 平均 sentiment を 1 行で
- **Mood meter**: ヘッダーに `⚠ TENSE` のような今の状態ラベル + **-1〜+1 の sentiment スライダー** (中央線 + 色付きマーカーがリアルタイムに左右に動く)。会議の「温度」がひと目で分かります
...
:::details エンジニア向け補足: ToneAgent のキャッシュ戦略と API
...
API は `GET /meetings/{id}/tone` の 1 本に集約していて、レスポンス 1 件で

- `per_utterance[]` — 各発言の感情ラベル + sentiment + speaker_name
- `participant_moods[]` — 話者別 dominant emotion + 直近 8 件の emotion stream + 平均 sentiment
- `overall_mood` + `overall_sentiment` — 会議全体の温度感

を全部返します。frontend は `LiveTranscript` / `ParticipantsPanel` / Mood meter の 3 箇所が同じ tone レスポンスを共有 (TanStack Query で de-dup) するので、4 秒間隔の polling 1 本で全部更新されます。
...
- **9 人の観察役 + 6 つの問いを通す司会補佐** という Agentic AI の作り方で、「いつ・誰に・どんな強さで助け舟を出すか」を、ちゃんと考えて出します。会議中に間に合わなかった懸念も、**最後のレポートには必ず残る** ようにしてあります。9 人の中には、**先週の決定を覚えていてくれる記憶係(Phase 7)** と、**発言テキストから会議の温度感を読む空気読み係(Phase 8、カメラ不使用)** も含まれています。
- **失敗も書きました**: カメラ路線(Phase 6)を 2,500 行書いてから撤去した話を §9.2 に正直に書いています。ハッカソンの記事って成功話ばかりになりがちですが、**何を作るべきじゃなかったか** も技術記事の正しい中身だと思っています。
...
開発初期はカメラ + MediaPipe FaceLandmarker でも作っていて、途中で撤去しています。詳しい経緯と 2 つの壁(C# media SDK 問題 / opt-in カメラ問題)は **§9.2 を読んでみてください** — 失敗から学んだ話としていちばん書きたかった部分です。
