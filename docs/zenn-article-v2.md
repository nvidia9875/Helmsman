...
- **どう作ったか**: Azure の上で **10 個の AI** がそれぞれ違う観点から会議を見ていて、「司会の補佐役」がそれをまとめて、ちょうどいい強さで助け舟を出します。強さは「ささやき」「カード」「声で発言」の 3 段階です。**先週の会議で何を決めたか覚えていてくれる「記憶 AI」** と、**カメラの表情から困惑・うなずきを読み取る「マルチモーダル AI」** も組み込みました(任意機能)。
...
| **空気読み係** 👀 (Phase 6) | 「○○さん、いま困った顔してました。説明をもう一回しましょうか?」と気づいてくれる人 |
...
| **Engagement Agent** (Phase 6) | **Rule-based** (顔シグナル閾値判定のみ) | (LLM 不要) |
...
async def run_tick(meeting: Meeting) -> list[InterventionCandidate]:
    coverage, steering, decision, quiet, dissent, memory, engagement = await asyncio.gather(
        ...
        engagement_agent.run(meeting),       # Phase 6 (rule-based、LLM 呼ばない)
    )
    timekeep = timekeeper.run(meeting)
    candidates = flatten([coverage, steering, decision, quiet, dissent, memory, engagement, timekeep])
    return arbiter.decide(candidates, now=meeting.audio_time)
```

実測平均レイテンシ **2.08 秒 / tick** (LLM 7 並列、20 秒に 1 回発火)。EngagementAgent は LLM を呼ばずに閾値判定だけなので、tick あたりのコスト追加は ~$0.0002 にとどまっています(Phase 6 で会議 1 本あたりの実コストは $0.03 → $0.04 程度)。
...
| **プライバシー** | 音声と文字起こしは **会議終了 30 日で自動で消える**。声紋データの保存は、ご本人がはっきり同意したときだけ。**Phase 6 で追加した顔シグナル機能 (任意) は、生フレームをサーバーに送らず、ブラウザ内で集計値だけ計算して送信** |
...
### 6.1 Phase 6 顔シグナル機能の具体的なプライバシー設計

Phase 6 で「うなずき / 困惑 / 集中度」を顔から読む機能を追加しましたが、ここはとくに慎重に設計しています。

- **デフォルト OFF**: Solo モードのカード上に Switch が出るだけで、ユーザが ON にしない限り起動しません
- **明示的な許可**: ON にした瞬間にブラウザのカメラ許可ダイアログが必ず出ます。許可しなければ何も動きません
- **ローカル処理**: フレーム解析は **すべてブラウザの中だけ** で完結 (MediaPipe FaceLandmarker、WASM)。サーバーには画像が一切届きません
- **集計値のみ送信**: 2 秒ごとに「うなずき回数 (整数) / 困惑度 (0-1) / 集中度 (0-1)」を計算 → 4 秒ごとに batch POST。元の画像から復元できない粒度に意図的に落としています
- **常時表示の「ON AIR」バッジ**: カメラ動作中は赤い点滅バッジが常に画面に出ます。「いま動いてるか」が分からない状態を作りません
- **1-click OFF**: Switch を OFF にするとプレビュー動画も消え、カメラ track が即座に release されます (デバイスインジケータも消える)
- **30 日 TTL**: 集計値も発言ログと同じ TTL ポリシーで自動削除

「カメラを使う AI」は、扱いを間違えると一気に怖いプロダクトになります。だからこそ「opt-in」「ローカル処理」「集計値のみ」の 3 つは妥協していません。
...
| Phase | 何をやるか | いつ |
|---|---|---|
| ~~**Phase 6: マルチモーダル**~~ | ~~表情も読み取って、「困ってる顔」「うなずき」も拾う~~ | ✅ **実装済 (2026-05)** |
| ~~**Phase 7: 会議をまたいで記憶する**~~ | ~~週次の連続会議で「あの時こう決めましたよね」を横断的に探せる~~ | ✅ **実装済 (2026-05)** |
| **Phase 8: Power Automate と仲良くなる** | 決定がそのまま Planner / Loop / DevOps の Work Item に流れる | 2026 Q4 |
| **Phase 9: 組織全体に広げる** | 「同じ論点を別チームでも話してるかも」に気づける | 2027 Q1 |
| **Phase 10: Multi-tenant SaaS / Marketplace** | 自社ドメイン許可で社内サブスクとして配布 | 2027 Q2 |

### 9.1 ✅ Phase 6: マルチモーダル(実装済 2026-05)

ブラウザの **MediaPipe FaceLandmarker** で会議参加者の顔から signal を読み取り、それを 10 番目の agent **EngagementAgent** に渡して介入候補を作ります。具体的にはこんな信号を見ています。

| 信号 | 何を見ているか | こんな時に AI が反応する |
|---|---|---|
| **うなずき (nod)** | head pitch の周期的な振動 (5-15°、0.5-2Hz) | 全員 nodding + 困惑なし → 「決まりそうですね、言葉にしておきますか?」 |
| **困惑 (confusion)** | 眉を寄せる + 唇を結ぶ + 鼻に皺 (ARKit blendshape の加重平均) | 困惑が続く + 60 秒沈黙 → 「もう少し聞きたい点ありますか?」(L2) |
| **集中度 (engagement)** | 目の開度 + 視線方向 (横向き角度ペナルティ) | 全体的に低下 + 同じ topic 5 分以上 → 「方向性を確認しましょうか?」 |

**プライバシー設計** をいちばん大事にしました。

- ✅ **デフォルト OFF**、ユーザが Switch を ON にしないと一切動かない
- ✅ **動画はサーバーに送らない**。フレーム解析は全部ブラウザの中だけ
- ✅ サーバーに届くのは「2 秒ごとの集計値」(うなずき回数 / 困惑度 0-1 / 集中度 0-1) のみ
- ✅ 30 日で自動削除 (発言ログと同じ TTL ポリシー)
- ✅ 1-click で OFF にでき、即座に カメラ track が解放される

### 9.2 ✅ Phase 7: 会議をまたいで記憶する(実装済 2026-05)
...
:::details エンジニア向け補足: Phase 8 で検討中の Hume EVI 統合 (未実装)
...
[Hume EVI (Empathic Voice Interface)](https://www.hume.ai/empathic-voice-interface) は会話の感情状態に応じて TTS prosody を動的に変調する API です。実装済 Phase 6 のマルチモーダル signal と組み合わせると、Helmsman の介入は次の 3 軸で会議と同期します。

- **何を言うか** ← Arbiter (6 段フィルタ)
- **誰に向けて言うか** ← 代弁構造 + L1/L2/L3 グラデーション + 顔の困惑度 (Phase 6 ✅)
- **どんな声色で言うか** ← Hume EVI (Phase 8 で検討中)

※ Phase 8 想定。API 検証 / コスト試算 / 倫理レビュー(感情応答 TTS は別の Responsible AI 課題を生む)が未完のためロードマップ補足扱いです。
...
- **10 人の観察役 + 6 つの問いを通す司会補佐** という Agentic AI の作り方で、「いつ・誰に・どんな強さで助け舟を出すか」を、ちゃんと考えて出します。会議中に間に合わなかった懸念も、**最後のレポートには必ず残る** ようにしてあります。10 人の中には、**先週の決定を覚えていてくれる記憶係(Phase 7)** と、**カメラから困惑顔やうなずきを読み取る空気読み係(Phase 6、opt-in)** も含まれています。
...
A. **完全に opt-in です**。デフォルトでは OFF で、ユーザが Switch を ON にしてカメラ許可を出した時だけ動きます。動画は **サーバーに一切送りません**。フレーム解析は全部ブラウザの中だけで完結し、サーバーには「2 秒ごとの集計値 (うなずき回数 / 困惑度 / 集中度)」しか流れません。1-click で OFF にできて、即座にカメラが解放されます。詳細は §11 Responsible AI へ。

### Q8. 「あの時こう決めましたよね」って、AI が間違って言うことありますか?
...
### Q6. なぜ Microsoft Copilot Studio の Multi-Agent を使わずに、自前で書いたんですか?
...
ただし **Phase 8 で「ホスト向けセットアップ Agent」として Copilot Studio を再登場させる** 予定です(会議の招待 URL を投げたら、自動で Bot 派遣 + Planner 紐付けまでやってくれる)。
...
> この記事に「いいね」してもらえると、実測データと Phase 6 以降のマルチモーダル版もお届けします。
>
