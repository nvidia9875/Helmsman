...
- 会議終了後は **自社議事録テンプレートを貼るだけで markdown レポート生成** (手書きメモも取込可。約 $0.01 / 6-10 秒、Cosmos に履歴永続化)
...
| 📝 **MeetingReportGenerator** | LLM | 会議終了後に template + memo + 会議結果から markdown レポート生成 | gpt-5.4 (HIGH) |
...
### 文書 RAG 検証 (戦略 Memo を同時投入したケース)

合成戦略 Memo (1 KB) を `--doc-text` で注入した eval (v4) では、本パイプライン
全体の **文書グラウンディング** が完全動作することを確認:

- **6/6 topics の `document_reference` が自動付与** (例: 「企画配分」→「3H コンテンツ戦略」)
- **DOC-6 矛盾警告が発火**: 文書と会議発言の食い違いを DecisionCapture が
  「⚠️ 文書と矛盾の可能性…」として介入文に明示
- 決定捕捉が **10 → 12 件**、介入が **15 → 17 件** に増加
- 追加コストは **$0.015** (文書 context token 分のみ)
